...
import { broadcastFrameToObserver, fullSyncToObserver, computeObserverPan } from "./socket-handler.js";

import { drawFrameOverlay, setFrameOverlayVisible } from "./overlay.js";
...
export class PhyVisionLayer extends foundry.canvas.layers.InteractionLayer {
    ...
    _activate() {
        ...
        const bounds = { x: this.frame.x, y: this.frame.y, height: this.frame.h, width: this.frame.w, };
        fullSyncToObserver(bounds);
    }

    _deactivate() {
        setFrameOverlayVisible(true);
        drawFrameOverlay(this.frame);
        super._deactivate();
        this.refreshVisuals();
    }
}
